...
#define FUNCTION_ID_TO_INDEX(BUFFER)\
inline int3 IDToIndex##BUFFER(in uint id)\
{ int3 offset; offset.x = id % BUFFER##Size.x; id /= BUFFER##Size.x; offset.y = id % BUFFER##Size.y; offset.z = id / BUFFER##Size.y; return offset + BUFFER##StartIndex; }\

#define ID_TO_INDEX(BUFFER, ID)\
IDToIndex##BUFFER(ID)\
